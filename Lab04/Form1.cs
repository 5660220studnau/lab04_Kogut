...
namespace Lab04
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            toolTip1.SetToolTip(button_fileKEYsave, "Відкрити");
            toolTip1.SetToolTip(button_fileKEYopen, "Зберегти");
        }
                ...
                if (Directory.Exists(dirOUTpath))
                {
                    if (File.Exists(fileKEYpath))
                    {
                        ...
                        if (lengthINfile == lengthKEYfile)
                        {
                            ...
                            labelFIleWithKeyLength.Text = ("Розмір файла з ключем: " + size1.ToString() + " байт");
                        }
                        else
                        {
